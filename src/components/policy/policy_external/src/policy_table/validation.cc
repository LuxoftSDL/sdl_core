...
bool VerifyPredefinedApp(ApplicationPolicies::value_type& app_policies) {
  ...
  if (!predefined_request_types.is_valid()) {
    SDL_WARN(app_id << " policy invalid RequestTypes will be cleaned.");
    ...
    SDL_WARN(app_id << " request types have cleaned up.");
  }
  return true;
}
...
bool ApplicationPoliciesSection::Validate() const {
  ...
  while (iter != end_iter) {
    ...
    if (app_request_types.is_omitted()) {
      SDL_WARN("RequestTypes omitted for "
    ...
    if (!app_request_types.is_valid()) {
      SDL_WARN("Invalid RequestTypes for " << app_id << " Will be cleaned up.");
      app_request_types.CleanUp();
      if (app_request_types.is_cleaned_up()) {
        if (PT_PRELOADED == pt_type) {
          SDL_ERROR("RequestTypes empty after clean-up for " << app_id
                                                             << " Exiting.");
          return false;
        }

        SDL_WARN("RequestTypes empty after clean-up for "
                 << app_id << " Will be replaced with default.");

        app_request_types = *apps[kDefaultApp].RequestType;
      }

      SDL_DEBUG("Clean up for " << app_id << " is done.");
    ...
    if (app_request_types.is_empty()) {
      SDL_WARN("RequestTypes is empty for " << app_id);
    }
