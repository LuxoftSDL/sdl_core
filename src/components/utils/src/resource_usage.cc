...
ResourseUsage* Resources::getCurrentResourseUsage() {
  ...
  if (false == GetProcInfo(pid_stats)) {
    SDL_ERROR(logger_, "Failed to get cpu proc info");
    return NULL;
  }
  MemInfo mem_info;
  if (false == GetMemInfo(mem_info)) {
    SDL_ERROR(logger_, "Failed to get memory info");
    return NULL;
  }
...
bool Resources::GetProcInfo(Resources::PidStats& output) {
  ...
  if (num_succes != 43) {  // 43 is number of iteams in Resources::PidStats
    SDL_ERROR(logger_, "Couldn't parse all iteams in /proc/PID/stat file");
    return false;
  }
  return true;
...
bool Resources::GetMemInfo(Resources::MemInfo& output) {
  ...
  if (false == GetProcInfo(pid_stat)) {
    SDL_ERROR(logger_, "Failed to get proc info");
    result = false;
  } else {
  ...
  if (0 == (proc_dir = opendir(proc))) {
    SDL_ERROR(logger_, "Unable to access to " << proc);
    result = false;
    return result;
  }
  if (0 == readdir(proc_dir)) {
    SDL_ERROR(logger_, "Unable to read : " << proc_dir);
    closedir(proc_dir);
    result = false;
    return result;
  }
  ...
  if (-1 == stat(as_path.c_str(), &st) || 0 == st.st_size) {
    SDL_ERROR(logger_, "Unable to stat : " << as_path.c_str());
    result = false;
    return result;
  }
  output = st.st_size;
